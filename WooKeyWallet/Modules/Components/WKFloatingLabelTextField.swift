...
public class WKFloatingLabelTextField: UITextField {
    ...
    open func titleHeight() -> CGFloat {
        if let titleLabel = titleLabel, let text = titleLabel.text, let font = titleLabel.font {
            let size = NSString(string: text).size(withAttributes: [.font: titleLabel.font])
